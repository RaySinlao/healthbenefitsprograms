$(document).ready(function() {
    ...
    $('.q1').click(function(e){ under64 = e.target.innerHTML; });

    $('.q2').click(function(e){ liveInUS = e.target.innerHTML; });
    ...
    function working() {
        ...
        if (next.length) {
            ...
            poof() // Cann this to the confetti.js
            setTimeout(function(){
              onlyOnKonami = true // Cann this to the confetti.js
            }, 2000)

            $('.progress_bar span').text('100% Complete');
            $('.progress_bar span').css({ width: '100%' });
        }
    ...
    function next(e) {
        ...
        if(e.target.className === 'q-btn q2'){
             poof() // Cann this to the confetti.js
            setTimeout(function(){
              onlyOnKonami = true // Cann this to the confetti.js
            }, 1)
        ...
        if(e.target.className === 'form_wrap'){
            ...
            if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test($('input[name=email]').val())){
               $('.err').fadeIn();
               $('.err').text('You have entered an invalid email address!');
        ...
        parent.fadeOut(function() {
            if (next.length) {
                ...
                $('.progress_bar span').text(Math.round(progress) + '% Complete');
