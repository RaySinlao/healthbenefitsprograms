(function ($) {
    ...
    $.fn.jbvalidator = function (options) {
        ...
        let errorMessages = {
            ...
            maxLength: "Please lengthen this to %s characters or less (you are currently using %s characters).",
            minLength: "Please lengthen this to %s characters or more (you are currently using %s characters).",
            ...
            groupRadioBox: "Please select at least %s options.",
            ...
            HTML5: {
                ...
                tooLong: "Please lengthen this to %s characters or less (currently using %s characters).",
                tooShort: "Please lengthen this to %s characters or more (currently using %s characters).",
        ...
        /**
         * run validate when on input / focusout
         */
        let run = function () {
        ...
        let showErrorMessage = function (el, message) {
            //console.log(options.invalidClass);
            
            if(el.type == 'radio'){
                $(el).parents().closest('.input-group').removeClass(options.validClass);
                $(el).parents().closest('.input-group').addClass(options.invalidClass);  
                $(el).parents().closest('.fieldset').find('legend').removeClass(options.validClass);
                $(el).parents().closest('.fieldset').find('legend').addClass(options.invalidClass);
            }else{
            ...
            if (options.errorMessage) {
                ...
                if(el.type == 'radio'){
                    group = $(el).parents().closest('.checkradio-wrapper');
                }
        ...
        let hideErrorMessage = function (el) {
            ...
            //for radio and checkbox group within input-group class
            if(el.type == 'radio'){
                $(el).parents().closest('.input-group').removeClass(options.invalidClass);
                $(el).parents().closest('.fieldset').find('legend').removeClass(options.invalidClass);
            }else{
        ...
        let validator = {
            ...
            groupRadioBox: function (el, event) {

                if (hasAttr(el, 'type', 'radio')) {

                    let radioGroup = $(el).closest('[data-radio-group]');
                    let mustCheckedCount = 1;//$(radioGroup).data('vMinSelect');
                    let checkedCount = radioGroup.find('input[type=radio]:checked').length;
                    let groupRequired = typeof $(radioGroup).data('vRequired') === "undefined" ? 0 : 1;

                    if (radioGroup) {

                        if (typeof event.originalEvent !== "undefined" && event.originalEvent.type === 'input') {
                            $(radioGroup).find('input[type=radio]').each((i, item) => {
                                $(item).trigger('input');
                            })
                        }
                        ...
                        if (checkedCount < mustCheckedCount && (groupRequired || checkedCount > 0)) {
                            if ($(el).prop('checked') === false) {
                                return errorMessages.groupRadioBox.sprintf(mustCheckedCount);
                            }
                        }
                    }
                }
