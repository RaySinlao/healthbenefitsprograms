...
// This call for the popup image dialog
...
async function updateLead(submitted) {
	await fetch(`${window.hbpHost}/leads`, {
		...
		body: JSON.stringify({
			...
			have_kaiser: $('input[name=health_insurance]:checked').val(),
...
$(function () {
	...
	$('#lp_form').submit(async function (e) {
		...
		if (validator.checkAll() == 0) {
			...
			var covid_19_tests = $('input[name=covid_19_tests]:checked').val();
			var health_insurance = $('input[name=health_insurance]:checked').val();
			var kaiser_test = $('input[name=kaiser_test]:checked').val();
			...
			_cio.identify({
				...
				address: address,
				gender: gender,
				covid_19_tests: covid_19_tests,
				health_insurance: health_insurance,
				kaiser_test: kaiser_test,
				offer: 'athometest',
				url_path: 'moreplans',
	...
	$('.health_insurance').click(function (e) {
		if ($(this).is(':checked')) {
			setTimeout(function () {
				$('#step-2').hide();
				$('#step-4').fadeIn();
	...
	$('.kaiser_test').click(function (e) {
		if ($(this).is(':checked')) {
			setTimeout(function () {
				...
				/*if (
					$('#test_yes').is(':checked') &&
					$('#health_insurance_kaiser').is(':checked') &&
